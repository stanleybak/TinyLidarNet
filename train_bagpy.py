### training code using bagpy instead of rosbag
...
inference_time_figure_path = './Figures/inference_time.png'
...
for pth in dataset_path:
    print(f"Processing dataset: {pth}")
    ...
    ldata = pd.read_csv(b.message_by_topic('Lidar'))
    ranges_np = ldata.filter(regex='ranges_.*').to_numpy()
    lidar_data = ranges_np[:,::down_sample_param]

    adata = pd.read_csv(b.message_by_topic('Ackermann'))
    angle_speed_np = adata[['drive.steering_angle', 'drive.speed']].to_numpy()
    servo_data = angle_speed_np[:, 0]
    speed_data = angle_speed_np[:, 1]
    max_speed = max(max_speed, np.max(speed_data))
...
plt.savefig(inference_time_figure_path)
